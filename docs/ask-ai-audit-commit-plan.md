...
- Replace the "must call knowledge for every claim / use tools before any claim" framing with:
  ...
  `search_markets`. Only read the user's portfolio/positions/risk when they ask about *their own*
...
## Phase 4 — Usability & chat-UX (match leading assistant UX)
...
- **P2:** only *failed* turns can retry; no "regenerate" on a successful answer. Reload is wired only for the error branch.
...
## Phase 6 — Product upgrades (optional, toward leading assistant UX)
