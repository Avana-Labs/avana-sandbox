...
const files=cp.execFileSync('git',['ls-files','--cached','--others','--exclude-standard'],{encoding:'utf8'}).trim().split('\n').filter(f=>/\.[cm]?[jt]sx?$/.test(f)&&fs.existsSync(f))
...
for(const file of files){
 ...
 for(const imp of ts.preProcessFile(source,true,true).importedFiles){
  const resolved=ts.resolveModuleName(imp.fileName,path.resolve(file),options,ts.sys).resolvedModule?.resolvedFileName
...
const rows=files.filter(f=>f.startsWith('app/')&&/\.tsx?$/.test(f)&&!test(f)&&!/(^|\/)(page|layout|route|loading|error|not-found|global-error|template|default|sitemap|robots|manifest|opengraph-image|twitter-image|icon|apple-icon)\./.test(f)).map(file=>({file,lines:fs.readFileSync(file,'utf8').split('\n').length,consumers:incoming.get(file)||[]})).filter(x=>x.consumers.every(test)).sort((a,b)=>b.lines-a.lines)
