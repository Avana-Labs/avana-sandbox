...
import { inkButton, mono, paper } from "@/lib/surfaces"
...
export function QuotaBanner({
  ...
  className,
  ...props
}: Omit<ComponentProps<"div">, "children" | "used" | "limit" | "unit" | "resetsIn" | "upgradeLabel" | "onUpgrade"> & {
  used: number
  limit: number
  unit: string
  resetsIn: string
  ...
  return (
    <div
      ...
      className={cn(paper, "flex w-full max-w-sm flex-col gap-2.5 rounded-2xl p-3.5", className)}
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={onUpgrade}
          className={cn(inkButton, "ms-auto flex h-7 items-center rounded-full px-3 text-xs font-normal")}
        >
          {upgradeLabel}
        </button>
      </div>
