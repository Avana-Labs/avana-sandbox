...
import { collapsePanel, field, mono, ShimmerLabel, SwapLabel } from "@/lib/surfaces"
  ...
  return (
    ...
    >
      <CollapsibleTrigger className="group/trigger text-foreground/55 hover:text-foreground/90 flex items-center gap-2 rounded-md py-1 text-[13.5px] transition-colors outline-none">
        ...
        <span className={cn(mono, "bg-foreground/[0.06] text-foreground/70 rounded-md px-1.5 py-0.5")}>{query}</span>
        <span className="ms-auto flex w-4 items-center justify-end">
          {!running && <CheckIcon className="fade-in zoom-in-90 animate-in size-3.5 text-emerald-500 duration-200" />}
        </span>
      </CollapsibleTrigger>
