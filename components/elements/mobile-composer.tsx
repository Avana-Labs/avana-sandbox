    ...
    >
      ...
      <div className="flex items-end gap-2">
        <button
          ...
          onClick={onAttach}
          disabled={!onAttach}
          className={cn(ghostButton, field, "size-9 shrink-0 disabled:pointer-events-none disabled:opacity-30")}
        >
        ...
        <div className={cn(field, "flex min-w-0 flex-1 items-center gap-2 rounded-[18px] px-3 py-2")}>
          ...
          />
          {value === "" && <MicIcon className="text-foreground/35 size-4 shrink-0" />}
        </div>
        ...
        >
          {running ? <SquareIcon className="size-3 fill-current" /> : <ArrowUpIcon className="size-4" />}
        </button>
      </div>

      {!keyboardOpen && <span aria-hidden className="bg-foreground/15 mx-auto h-1 w-28 rounded-full" />}

      {keyboardOpen && <span className={cn(mono, "text-foreground/25 text-center")}>return to send</span>}
    </div>
  )
}
