...
import { field, mono, paper, ShimmerLabel } from "@/lib/surfaces"
...
export function RetrievalChunks({
  ...
  className,
  ...props
}: Omit<ComponentProps<"div">, "children" | "query" | "chunks" | "visibleCount" | "searching"> & {
  query: string
  ...
  return (
    ...
    >
      ...
      <div className="text-foreground/45 text-xs">
        {searching ? (
          <ShimmerLabel className="relative inline-block leading-none">Retrieving</ShimmerLabel>
        ) : (
          <span className="fade-in animate-in duration-300">{chunks.length} passages above threshold</span>
        )}
      </div>
        ...
        {take(chunks, visibleCount).map((chunk) => (
          ...
          >
            <div className="flex items-baseline gap-2">
              <span className="text-foreground/90 min-w-0 flex-1 truncate text-[13px] font-medium">{chunk.source}</span>
              <span className={cn(mono, "text-foreground/30 shrink-0")}>{chunk.locator}</span>
              <span
                className={cn(
                  mono,
                  "shrink-0 tabular-nums",
                  chunk.score >= 0.8 ? "text-emerald-600 dark:text-emerald-400" : "text-foreground/35",
                )}
              >
                {chunk.score.toFixed(2)}
              </span>
            </div>
            <p className="text-foreground/55 line-clamp-2 text-xs leading-relaxed">{chunk.text}</p>
