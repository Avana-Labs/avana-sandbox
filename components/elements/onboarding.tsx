...
export function Onboarding({
  ...
  className,
  ...props
}: Omit<ComponentProps<"div">, "children" | "steps" | "index" | "onNext" | "onSkip"> & {
  ...
  return (
    <div
      ...
      className={cn(paper, "flex w-full max-w-sm flex-col gap-4 rounded-[20px] p-5", className)}

      {...props}
    >
      <div key={current} className="fade-in animate-in flex flex-col gap-2 duration-300">
        <span className={cn(mono, "text-foreground/30 tabular-nums")}>
          {current + 1} of {steps.length}
        </span>
        <span className="text-[15px] font-medium tracking-tight">{step.title}</span>
        <p className="text-foreground/55 text-[13px] leading-relaxed break-words">{step.body}</p>
        <span className={cn(field, "text-foreground/60 rounded-xl px-3 py-2 text-[13px] leading-relaxed")}>
          {step.example}
        </span>
      </div>
