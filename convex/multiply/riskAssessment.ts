/**
 * Multiply product — Risk Premium / assessment card.
 * Table: `multiplyRiskAssessments` (slug-keyed; not shared with borrow/lend).
 * Distinct from `multiplyRiskParameters` (parameter grid).
 */
...
export const upsertRiskAssessments = internalMutation({
  ...
  handler: async (ctx, { rows }) => {
    for (const row of rows) {
      ...
      else await ctx.db.insert("multiplyRiskAssessments", row)
    }
