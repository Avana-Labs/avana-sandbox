/**
 * Multiply product — About / FAQs / parameter-change history.
 * Table: `multiplyMarketContent` (slug-keyed; not shared with borrow/lend).
 */
...
export const upsertContent = internalMutation({
  ...
  handler: async (ctx, { rows }) => {
    for (const row of rows) {
      ...
      else await ctx.db.insert("multiplyMarketContent", row)
    }
