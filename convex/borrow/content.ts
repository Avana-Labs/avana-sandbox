/**
 * Borrow product — About / FAQs / parameter-change history.
 * Table: `borrowMarketContent` (slug-keyed; not shared with lend/multiply).
 */
...
export const getContent = query({
  ...
  handler: async (ctx, { slug }) => {
    ...
    return {
      ...
      history: row.history,
      faqs: row.faqs,
      changelog: await readChangelog(ctx, "borrow", slug),
    }
  },
})
...
export const upsertContent = internalMutation({
  ...
  handler: async (ctx, { rows }) => {
    for (const row of rows) {
      ...
      else await ctx.db.insert("borrowMarketContent", row)
    }
