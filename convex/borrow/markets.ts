/**
 * Borrow product — market identity (pool + asset).
 * Table: `borrowMarkets`. Legacy `markets` remains the FK hub for walletEvents.
 */
...
export const upsertMarkets = internalMutation({
  ...
  handler: async (ctx, { rows }) => {
    for (const row of rows) {
      ...
      else await ctx.db.insert("borrowMarkets", row)
    }
