...
describe.skipIf(process.env.RUN_ONBOARDING_10K !== "1")("sandbox onboarding — exact 10,000-wallet capacity", () => {
  test(
    "onboards every wallet, grants exactly $1M, and preserves wallet isolation",
    async () => {
      ...
      const persisted = await t.run(async (ctx) => {
        ...
        return {
          ...
          economyUsers:
            (economy?.userCount ?? 0) + shards.reduce((sum, shard) => sum + shard.userCount, 0),
          economyGrantedUsd:
            (economy?.totalGrantedUsd ?? 0) + shards.reduce((sum, shard) => sum + shard.grantedUsd, 0),
        }
      })
