 ...
 * (supply). Preloads it once, builds the initial `heroFeed` from the preloaded value (no
 * second fetch), and returns the `Preloaded` token so the live hero uses `usePreloadedQuery`
 * (hydrate + subscribe, never re-fetch). `preloads: null` when no deployment URL is set.
 */
