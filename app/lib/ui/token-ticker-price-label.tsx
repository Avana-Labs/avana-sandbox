...
/**
 * Line under a token name in a table row. Touch / phone: `price`. Desktop (hover-capable, md+): the
 * ticker, which flips over to `price` while the row is hovered — the row must carry Tailwind's
 * `group` class. Without a price it shows the ticker everywhere.
 */
export function TickerPriceFlip({ symbol, price }: { symbol: string; price: string | undefined }) {
  const ticker = formatTokenDisplaySymbol(symbol)
  if (price === undefined) return <>{ticker}</>
  return (
    <>
      <span className="block truncate [@media(hover:hover)_and_(min-width:768px)]:hidden">{price}</span>
...
export function TokenTickerPriceLabel({ symbol }: { symbol: string }) {
  ...
  return <TickerPriceFlip symbol={symbol} price={price === undefined ? undefined : formatTokenPrice(price)} />
}
