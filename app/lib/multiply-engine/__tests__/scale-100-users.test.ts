...
describe("multiply system 100-user scale", () => {
  it("settles 100 heterogeneous wallet sessions while keeping reads and invariants consistent", { timeout: 30_000 }, () => {
    ...
    const whale = Object.values(nextState.positions).find((position) => position.walletId === "wallet-multiply-stress-0")
    const regular = Object.values(nextState.positions).find((position) => position.walletId === "wallet-multiply-stress-7")
    if (whale && regular) {
