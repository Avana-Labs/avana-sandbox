...
describe("isAnalyticsBeaconRejection", () => {
  it("matches the Vercel analytics beacon 'Failed to fetch' rejection", () => {
    expect(
      isAnalyticsBeaconRejection(new TypeError("Analytics SDK: Failed to fetch (AnalyticsSDKApiError)")),
    ).toBe(true)
    expect(isAnalyticsBeaconRejection("Analytics SDK: TypeError: Failed to fetch")).toBe(true)
    expect(
      isAnalyticsBeaconRejection(new TypeError("Failed to fetch /_vercel/insights/event")),
    ).toBe(true)
