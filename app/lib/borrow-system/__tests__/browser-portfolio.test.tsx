...
describe("browser portfolio integration", () => {
  it("matches read adapter totals after borrow execute and rehydrate", async () => {
    ...
    const snapshot = await readAdapter.readWalletSnapshot("wallet-1")

    expect(portfolio.debtPositions[0]?.borrowedUsd).toBeGreaterThan(4200)
    expect(snapshot.transactionHistory.at(-1)?.hash).toMatch(/^sim/)
