    ...
    <div className="rounded-radius-md bg-card px-4 py-4">
      <div className="flex min-w-0 items-center gap-2.5">
        ...
        <div className="min-w-0">
          <div className="truncate text-base font-normal tracking-[-0.03em]">{card.asset}</div>
          <div className="truncate text-sm text-muted-foreground">{card.coverage}</div>
        </div>
      </div>

      <div className={cn("mt-3 text-xs font-normal uppercase tracking-wide", statusTone)}>
        {t(statusLabelKey(card.cooldownStatus))}
      </div>

      <div className="mt-2 grid grid-cols-2 gap-2">
        <div>
          <div className="text-sm text-muted-foreground">{t("Cooldown")}</div>
          <div className="mt-1 text-xl font-normal tracking-[-0.04em] tabular-nums">{cooldownLabel}</div>
        </div>
        <div>
          <div className="text-sm text-muted-foreground">{t("Removes in")}</div>
          <div className={cn("mt-1 text-xl font-normal tracking-[-0.04em] tabular-nums", removesTone)}>
            {removesLabel}
          </div>
        </div>
      </div>
