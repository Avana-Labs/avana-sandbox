...
export function KeyMetricsCard({ detail }: Props) {
  ...
  const chips = React.useMemo(
    () => allChips.filter((c) => SECONDARY_METRICS.includes(c.id)).map((c) => ({ ...c, label: t(c.label) })),
    [allChips, t],
  )
  ...
  return (
    <SectionCard
      title={t("Deeper metrics")}
      subtitle={t("Secondary indicators the risk council reviews for this pool.")}
      bodyClassName="p-0"
