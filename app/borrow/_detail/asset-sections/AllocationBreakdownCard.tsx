...
function marketHrefForAllocationRow(assetId: string, rowId: string) {
  ...
  return rowId.startsWith(prefix)
    ? borrowMarketDetailPath(rowId.slice(prefix.length))
    : borrowMarketDetailPath(rowId)
}
...
export function AllocationBreakdownCard({ detail, id }: Props) {
  ...
  const helpText = t("Where {symbol} is deployed across LP collateral pools.").replace(
    "{symbol}",
    detail.hero.symbol,
  )
