...
/**
 * The wagmi-backed body of the wrong-network banner. Statically imports `useWrongNetwork`
 * (a wagmi hook), so it lives in its own module and is dynamically loaded only once the
 * wallet SDK is mounted — keeping wagmi off the guest critical path. Renders nothing on the
 * correct network.
 */
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="text-sm font-normal">{t("Wrong network")}</div>
        <div className="mt-0.5 text-sm leading-relaxed opacity-90">
          {t("Switch your wallet to {chain} to continue.").replace("{chain}", targetChainName)}
        </div>
        {switchError ? <div className="mt-1 text-xs text-rose-600 dark:text-rose-300">{switchError}</div> : null}
      </div>
      <button
        type="button"
        onClick={() => void switchToTargetChain()}
        disabled={isSwitching}
        className="inline-flex h-9 shrink-0 items-center justify-center rounded-full bg-amber-500 px-4 text-sm font-normal text-amber-950 transition-colors hover:bg-amber-500/90 disabled:opacity-70"
      >
