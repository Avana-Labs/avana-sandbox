...
const TESTNET_LABEL_CLASS = "text-[11px] leading-none tracking-[0.02em] text-brand"
...
function MarkWithLabel({ className }: { className?: string }) {
  ...
  return (
    <span className={cn("inline-flex flex-col items-center gap-0.5", className)}>
      <BrandIcon />
      <span className={TESTNET_LABEL_CLASS}>{t("Testnet")}</span>
    </span>
  )
}
  ...
  return (
    <>
      ...
      <span className="hidden flex-col items-start gap-0.5 xl:inline-flex">
        <BrandLogo />
        <span className={TESTNET_LABEL_CLASS}>{t("Sandbox · testnet")}</span>
      </span>
    </>
  )
}
