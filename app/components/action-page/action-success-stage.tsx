    ...
    <div data-testid="action-success-stage" className="space-y-4">
      {receipt ? (
        <ActionCard className="overflow-hidden">
          <div className="relative px-4 pb-2 pt-4">
            ...
            <div className="flex flex-col items-center py-6 text-center">
              ...
              <h2 className="mt-4 text-[1.25rem] font-medium tracking-[-0.03em]">{success.title}</h2>
              <p className="mt-1.5 max-w-sm text-[14px] text-muted-foreground">{success.description}</p>
              {success.receiptHash ? (
            ...
            <div className="divide-y divide-border border-t border-border">
              ...
              <ActionInfoRow label="Market" value={receipt.marketValue} tooltip="market" />
            </div>
          </div>
        </ActionCard>
      ) : (
        <div className="flex flex-col items-center py-4 text-center">
          ...
          <h2 className="mt-3 text-[18px] font-medium tracking-[-0.02em]">{success.title}</h2>
          <p className="mt-1.5 max-w-sm text-[14px] text-muted-foreground">{success.description}</p>
          {success.receiptHash ? (
            <p className="mt-2 font-data text-[12px] text-muted-foreground">
              Receipt: <span className="text-foreground">{success.receiptHash}</span>
