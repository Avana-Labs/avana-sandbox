  ...
  return (
    <ActionPageShell title={title} subtitle={undefined} closeHref={closeHref} simulated={false}>
      <div className="rounded-radius-md border-0 bg-card p-6 text-center" data-testid="action-not-found">
        <h2 className="text-lg font-normal text-foreground">{t(title)}</h2>
        <p className="mt-2 text-sm text-muted-foreground">{t(message)}</p>
        <Link
          href={closeHref}
          className="mt-5 inline-flex h-11 items-center justify-center rounded-full bg-foreground px-5 text-base font-normal text-background transition-opacity hover:opacity-90"
        >
          {t("Go back")}
        </Link>
      </div>
