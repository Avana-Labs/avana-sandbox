  ...
  return (
    <>
      <button
        ...
        className={cn(
          "inline-flex shrink-0 items-center gap-1.5 rounded-full border border-border px-3 py-1.5 text-sm font-normal text-foreground transition-colors hover:bg-hover",
          className,
        )}
      >
        {t("How it works")}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[min(620px,calc(100dvh-96px))] w-full max-w-[500px] gap-0 overflow-hidden rounded-radius-xl border-border bg-background p-0 shadow-[0_24px_80px_rgba(15,23,42,0.22)] sm:w-[calc(100vw-24px)] [&>button]:right-3.5 [&>button]:top-3.5 [&>button]:rounded-full">
          <div className="border-b border-border px-5 py-4 pr-12">
            <DialogTitle className="text-lg font-normal tracking-[-0.02em] text-foreground">
              {content.title}
            </DialogTitle>
          ...
          <div className="max-h-[min(460px,calc(100dvh-220px))] space-y-4 overflow-y-auto px-5 py-4">
            {content.sections.map((section) => (
              <div key={section.heading}>
                <div className="text-sm font-normal text-foreground">{section.heading}</div>
