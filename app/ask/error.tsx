...
export default function AskAIError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <main className="flex min-h-[calc(100dvh-56px)] items-center justify-center px-6 py-12">
      <div className="max-w-md rounded-2xl border border-border bg-card p-8 text-center">
        <h1 className="text-xl font-normal tracking-[-0.02em]">Ask AI couldn&apos;t load</h1>
        ...
        <button
          type="button"
          onClick={reset}
          className="mt-6 rounded-full bg-brand px-5 py-2.5 text-sm font-normal text-brand-foreground"
        >
          Try again
        </button>
      </div>
